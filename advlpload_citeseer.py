...
def train(args):
    ...
    optimizer_en = getattr(optimizers, args.optimizer)(params=model.encoder.parameters(), lr=args.lr,
                                                    weight_decay=args.weight_decay)
    optimizer_de = getattr(optimizers, args.optimizer)(params=model.decoder.parameters(), lr=args.lr,
                                                       weight_decay=args.weight_decay)
    ...
    # # if epoch%3==2:
        ...
        if (epoch + 1) % args.eval_freq == 0:
            ...
            if model.has_improved(best_val_metrics, val_metrics1):
                ...
                best_test_metrics1 = model.compute_metrics(embeddings, data, 'test')
